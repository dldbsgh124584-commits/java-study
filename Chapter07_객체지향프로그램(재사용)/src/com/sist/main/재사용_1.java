 ...
 *           => 다른 메소드를 이용할 수 있따
 *        => 캡슐화는 데이터를 감추고 메소드를 이용해서 접근하는 기술 (안정성 / 유지보수를 높이는 기술)
 *           ** 웹에서 주로 사용되는 기술
 *           ** 웹에서는 시큐어 코딩 (private)
 *                     --------
 *                      | => 3천만원 벌금
 ...
 *           3. 상속의 키워드 => extends
 *        2) 특징
 *           1. 단일 상속 (다중 상속을 하게 되면 => 중복된 메소드,변수 가 있어서 충돌이 될 수 있다 => 다중 상속을 하려면 인터페이스를 사용한다)
 *           2. 상속 예외
 *               = 생성자
 *               = 초기화 블록
 *               = static
 ...
 *   
 *   => 상속 내리는 클래스 > 상속 받는 클래스  - 내리는 쪽이 더 크다
 ...
 *           int a,b,c;
 *         }
 *         --------------- 비교 불가능 (변수 갯수가 많다고 큰게 아니다 상속이 있어야 비교가 가능해짐)
 *           동물
 *            |
 *           사람 => 클래스가 커진다
 *               => 사람은 동물이다 => is-a
...
{
   //int a,b,c; 눈에 보이진 않지만 메모리 상에 저장 (생성자,초기화블록,static 은 x)
