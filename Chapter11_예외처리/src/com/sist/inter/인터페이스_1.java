 ...
 *         ----------------------------------
 *       역할
 *        1. 서로 다른 클래스를 연결해서 사용 : 결합성을 낮게 함
 *        2. 관련 클래스를 모아서 한개의 이름으로 제어
 *           => 웹, 스프링
 *        3. 설계와 관련 => 기능 설계
 *        4. 모든 개발자가 같은 메소드를 구현 => 표준화
 *           => 소스 통일화
 *        5. 요구사항 분석 => 기능
 *       -------------------------------------------
 *       => 표준화
 ...
 *          ------------------------
 *          => 인터페이스 => 구성요소가 only public 
 *          => 자체 처리인 경우 : private 메소드가 가능 
 *       }
 *       
 *       ==> new를 사용할 수 없다
 *       ==> 사용기 가능
...
interface 도형
{
	...
	/*
	 *  abstract public void draw(); 접근지정어와 제어어 순서가 바뀌어도 된다
	 * 
	 */
}
